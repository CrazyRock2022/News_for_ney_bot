...
# Инициализация токенов и API ключей
...
# Фиксированные темы для фильтрации новостей (по шаблону)
FIXED_TOPICS = [
    "A7A5", "Кыргызстан", "kyrgyzstan", "крипта", "crypto", "цифровой рубль",
    "digital ruble", "стейблкоин", "stablecoin", "CBDC", "digital currency"
]
...
def init_db():
    ...
    # начальные источники
    default_sources = [
        'https://forklog.com/feed/', 'https://ru.cointelegraph.com/rss', 
        'https://bits.media/rss/news/', 'https://incrypted.com/feed/', 
        'https://cryptopanic.com/news/rss/', 'https://cointelegraph.com/rss', 
        'https://decrypt.co/feed', 'https://www.coindesk.com/arc/outboundfeeds/rss/?outputType=xml', 
        'https://www.cbr.ru/rss/', 'http://www.finmarket.ru/rss/', 
        'https://rssexport.rbc.ru/rbcnews/news/eco/index.rss', 'https://www.kommersant.ru/RSS/news.xml', 
        'https://www.forbes.ru/rss', 'https://24.kg/rss/', 'https://akipress.org/rss/news.rss', 
        'https://www.themoscowtimes.com/rss', 'https://blogs.imf.org/feed/', 'https://www.bis.org/rss/home.xml'
    ]
...
# ---------- GPT-ФИЛЬТР ----------
async def is_relevant(title, summary, tags=None, category=None, content=None, custom_prompt=None):
    if custom_prompt:
        prompt = custom_prompt
    else:
        # Default prompt (based on predefined topics)
