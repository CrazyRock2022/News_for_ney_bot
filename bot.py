...
async def is_relevant(title, summary, tags=None, category=None, content=None):
    ...
    if category:
        full_context += f"Категория: {category}\n"
    if tags:
        full_context += f"Теги: {', '.join(tags)}"
    if content:
        full_context += f"\nПолный текст: {content[:1000]}..."

    prompt = (
        f"Ты аналитик криптовалютного проекта A7A5. "
        f"Проанализируй новость и ответь, может ли она быть релевантной проекту A7A5, если она касается криптовалют, "
        f"стейблкоинов, цифрового рубля, экономики Кыргызстана, финансовых регуляторов, или мировой криптоинфраструктуры. "
    ...
    for url in sources:
        ...
        for entry in feed.entries:
            ...
            if published:
                ...
                if datetime.utcnow() - pub_time > timedelta(hours=168):  # За последние 7 дней
                    continue
